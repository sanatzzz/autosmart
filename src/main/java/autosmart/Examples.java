...
public class Examples {
	...
	private void demonstrateMorphologicalAnalysis(String phrase) throws JWNLException {
		...
		System.out.println("Base form for \"" + phrase + "\": " + dictionary.lookupIndexWord(POS.VERB, phrase));
	}
	...
	private void demonstrateAsymmetricRelationshipOperation(IndexWord start, IndexWord end)
		...
		// Try to find a relationship between the first sense of <var>start</var> and
		// the first sense of <var>end</var>
		RelationshipList list = RelationshipFinder.findRelationships(start.getSenses().get(0), end.getSenses().get(0),
